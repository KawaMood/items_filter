...
execute if score $is_loaded pk.temp matches 0 run schedule function pk_it_fi:tools/debug/recreate_all_blocks/3 1s
execute if score $is_loaded pk.temp matches 0 run return fail

# @continue If yes, recreate current items filter
$execute unless score $is_loaded pk.temp matches 0 in $(dimension) positioned $(x).0 $(y).0 $(z).0 run function pk_it_fi:tools/debug/recreate_all_blocks/id/$(block_id)
...
# Continue process with next items filter to recreate, or stop it if there are no items filters anymore
...
execute if score $pk.debug.recreate.blocks.length pk.value matches 0 as @a[tag=pk.debug.recreate,limit=1] run function pk_it_fi:tools/debug/recreate_all_blocks/stop
execute unless score $pk.debug.recreate.blocks.length pk.value matches 0 run function pk_it_fi:tools/debug/recreate_all_blocks/1