...
# Debug logs
execute if score $logs.datapack.it_fi pk.value matches 1 run tellraw @a[tag=pk.dev] [{text:"Items Filter has been moved to [",color:"gray"},{nbt:"temp.location.x",storage:"pk:common",interpret:true},{text:","},{nbt:"temp.location.y",storage:"pk:common",interpret:true},{text:","},{nbt:"temp.location.z",storage:"pk:common",interpret:true},{text:"]"}]