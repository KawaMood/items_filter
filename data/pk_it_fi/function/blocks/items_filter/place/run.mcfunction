...
# Debug logs
execute if score $logs.datapack.item_filters pk.value matches 1 run tellraw @a[tag=pk.dev] [{"selector":"@a[tag=pk.current.player,limit=1]",color:"gray"},{text:" placed Items Filter at ["},{nbt:"temp.items_filter.location.x",storage:"pk:common",interpret:true},{text:","},{nbt:"temp.items_filter.location.y",storage:"pk:common",interpret:true},{text:","},{nbt:"temp.items_filter.location.z",storage:"pk:common",interpret:true},{text:"] in "},{nbt:"temp.items_filter.location.dimension",storage:"pk:common",interpret:true}]