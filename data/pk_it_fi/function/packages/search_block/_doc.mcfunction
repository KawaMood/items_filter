...
# Example:
data modify storage pk:common params set value {distance:5,block:"#pk_it_fi:player_heads",nbt:"",callback:"setblock ~ ~ ~ stone",stop_at_first:true}
function pk_namespace:packages/search_block/start with storage pk:common params
# Would search a block matching the tag #pk_it_fi:player_heads in a 10x10x10 area where the current location is the center
