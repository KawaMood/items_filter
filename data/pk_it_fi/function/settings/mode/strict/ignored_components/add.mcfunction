...
execute unless score $already_contains pk.temp matches 1.. run tellraw @s [{text:"Added \"",color:"gray"},{nbt:"temp.input.id",storage:"pk:common",interpret:true,color:"yellow"},{text:"\" to components ignored by the strict mode",color:"gray"}]
