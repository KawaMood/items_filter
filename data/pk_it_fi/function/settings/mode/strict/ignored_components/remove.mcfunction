...
# If found
execute if score $found pk.temp matches 1.. run tellraw @s [{text:"Removed \"",color:"gray"},{nbt:"temp.input.id",storage:"pk:common",interpret:true,color:"yellow"},{text:"\" from components ignored by the strict mode",color:"gray"}]
